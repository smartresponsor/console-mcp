...
`go <component> M<N>` (engine-backed `executorMode: "engine"` path of `console.write.browser.session.cmcp.go`, implemented as `executeEngineBackedCmcpGo` in `src/tool/chatgpt-chat-open.ts`) authorizes the task with a protective five-iteration floor: omitted `M` defaults to `M5`, `M1`–`M4` normalize to `M5`, `M5` remains `M5`, and `M6+` preserves the explicitly requested larger budget. The normalized value is persisted as `max_auto_iterations` before `workerTick` drives the local 7-phase `REPO_RC_PHASE_PLAN` (reconnaissance → workspace_state → boundary_policy → implementation_plan → patch_materialization → gate_execution → status_report — this phase plan is local bookkeeping in `src/engine/engine-core.ts` and issues no browser calls). Once the plan reaches its last phase with `execution_authorized=true`, `workerTick` sets task status to `done` and emits `task_phase_plan_complete_dispatch_ready`.
...
End-to-end result for `go <component> [M<N>]`: budget normalization (`max(5, N)`, with omitted `M` = 5) → authorization → phase plan (workerTick) → automatic `run_n` dispatch with `maxRounds` equal to that normalized durable budget, in one call. The dispatch result is reported back under `engine.run_n` in the `go` response; a `null` value there means the phase plan did not reach dispatch-ready (e.g. workspace missing), and `{ok:false, status:"ENGINE_CYCLE_RUN_N_DISPATCH_SKIPPED", ...}` means it reached done but a precondition (authorization or `max_auto_iterations`) wasn't met.
...
The adoption path now shares the same bounded engine-cycle contract. `console.write.browser.chatgpt.chat.adopt_into_task_bank` remains adoption-only by default (`autoStart=false`): it binds the existing conversation, records the adoption continuation specification, and returns `console.write.engine.cycle.run_n` as the explicit next tool. `ADOPT GO` (`console.write.browser.chatgpt.chat.adopt_go`) forces `autoStart=true`, authorizes with `authorized_by: "adopt"`, advances the local phase plan, and—when `manageLoop=true`—dispatches the same `runEngineCycleRounds` implementation used by CMCP Go. Its reasoning model/effort/enforcement inputs match CMCP Go. Because Adopt does not receive a free-form CMCP command from which authority can be inferred, it also exposes explicit `executionAuthority=read_only|write_allowed`; that value is materialized into the authoritative continuation specification so the RunSpec mutation policy remains durable. Durable `max_auto_iterations` / `auto_iteration_count`, semantic-stall detection, completion verification, lease protection, NOT_READY handling, and stop-before-reply budget enforcement therefore apply identically after the existing chat is adopted. Setting `manageLoop=false` is the same inspection escape hatch: adoption and authorization proceed, but the browser round loop is not dispatched automatically. Likewise, preparing a task without execution authorization still stops at `waiting_user` once the phase plan completes.
