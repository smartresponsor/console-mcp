...
function buildRepoRcPrompt(rawPrompt: string, workspacePath: string | null, componentName: string | null): string {
  ...
  return [
    ...
    "",
    "Intro spec:",
    "- This prompt was expanded by the console-mcp entrypoint planner from a shorter user request.",
    "- Treat the expanded prompt as the authoritative execution contract for this run.",
    "- Preserve the user's original intent, but apply all safety, scope, reconnaissance, helper-stack, and runner defaults below.",
    "- Do not skip reconnaissance because the original user request was short.",
    "- Do not broaden the target bounded context even when related repositories or helper stacks are inspected.",
    "- Report blockers precisely instead of guessing or inventing repository state.",
    "",
