...
async function refreshChatGptTitleState(target: ChatGptTarget, desiredTitle: string, timeoutMs: number): Promise<Record<string, unknown>> {
  ...
  const beforeTitle = asString(beforeRecord.document_title);
  const afterTitle = asString(afterRecord.document_title);
...
async function attemptChatGptSidebarUiRename(target: ChatGptTarget, desiredTitle: string, timeoutMs: number): Promise<Record<string, unknown>> {
  ...
  const attempt = await safeEvaluateInTarget(target.web_socket_debugger_url, buildSidebarUiRenameExpression(chatId, desiredTitle), Math.min(Math.max(timeoutMs, 5000), 15000), "CHAT_TITLE_UI_RENAME_EVALUATION_FAILED");
  await delay(1000);
  ...
  return { ok: confirmed, status: confirmed ? "CHAT_TITLE_UI_RENAME_CONFIRMED" : "CHAT_TITLE_UI_RENAME_NOT_CONFIRMED", chat_id: chatId, desired_title: desiredTitle, attempt, refresh_confirmation: refreshConfirmation };
}
...
function buildSidebarUiRenameExpression(chatId: string, title: string): string {
  const safeChatId = JSON.stringify(chatId);
  const desiredTitle = JSON.stringify(title);
  return `(async () => { const chatId = ${safeChatId}; const desiredTitle = ${desiredTitle}; const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms)); const clean = (value) => String(value || '').replace(/\\s+/g, ' ').trim(); const visible = (node) => { if (!node || !(node instanceof Element)) return false; const rect = node.getBoundingClientRect(); const style = getComputedStyle(node); return rect.width > 0 && rect.height > 0 && style.visibility !== 'hidden' && style.display !== 'none' && style.opacity !== '0'; }; const setValue = (node, value) => { node.focus && node.focus(); if (node instanceof HTMLInputElement || node instanceof HTMLTextAreaElement) { const proto = node instanceof HTMLTextAreaElement ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype; const descriptor = Object.getOwnPropertyDescriptor(proto, 'value'); if (descriptor && descriptor.set) descriptor.set.call(node, value); else node.value = value; node.dispatchEvent(new InputEvent('input', { bubbles: true, cancelable: true, inputType: 'insertText', data: value })); node.dispatchEvent(new Event('change', { bubbles: true, cancelable: true })); return true; } if (node.getAttribute && node.getAttribute('contenteditable') === 'true') { node.textContent = value; node.dispatchEvent(new InputEvent('input', { bubbles: true, cancelable: true, inputType: 'insertText', data: value })); return true; } return false; }; const anchor = Array.from(document.querySelectorAll('a[href*=\"/c/\"], a[href*=\"/chat/\"]')).find((node) => String(node.href || node.getAttribute('href') || '').includes(chatId)); if (!anchor) return { ok: false, status: 'CHAT_TITLE_UI_RENAME_ANCHOR_NOT_FOUND', chat_id: chatId, desired_title: desiredTitle, href: location.href, title: document.title }; let row = anchor; for (let index = 0; index < 8 && row.parentElement; index += 1) { row = row.parentElement; if (Array.from(row.querySelectorAll('button')).filter(visible).length > 0) break; } anchor.scrollIntoView({ block: 'center' }); const rowButtons = Array.from(row.querySelectorAll('button')).filter(visible); const menuButton = rowButtons.find((node) => /more|menu|options|actions|open/i.test(String(node.getAttribute('aria-label') || node.getAttribute('title') || node.dataset?.testid || ''))) || rowButtons[rowButtons.length - 1] || null; if (!menuButton) return { ok: false, status: 'CHAT_TITLE_UI_RENAME_MENU_NOT_FOUND', chat_id: chatId, desired_title: desiredTitle, row_text: clean(row.innerText || row.textContent).slice(0, 300), href: location.href, title: document.title }; menuButton.click(); await delay(600); const menuItems = Array.from(document.querySelectorAll('[role=\"menuitem\"], button, [data-radix-collection-item], [cmdk-item], [role=\"option\"]')).filter(visible); const renameItem = menuItems.find((node) => /rename|edit name|edit title/i.test(clean(node.innerText || node.textContent || node.getAttribute('aria-label') || node.getAttribute('title') || ''))); if (!renameItem) return { ok: false, status: 'CHAT_TITLE_UI_RENAME_ACTION_NOT_FOUND', chat_id: chatId, desired_title: desiredTitle, menu_text_sample: menuItems.map((node) => clean(node.innerText || node.textContent || node.getAttribute('aria-label') || '')).filter(Boolean).slice(0, 20), href: location.href, title: document.title }; renameItem.click(); await delay(700); const editors = Array.from(document.querySelectorAll('input, textarea, [contenteditable=\"true\"]')).filter(visible); const active = document.activeElement && visible(document.activeElement) ? document.activeElement : null; const editor = (active && (active instanceof HTMLInputElement || active instanceof HTMLTextAreaElement || active.getAttribute('contenteditable') === 'true') ? active : null) || editors.find((node) => node.closest('[role=\"dialog\"], [role=\"menu\"], [data-radix-popper-content-wrapper]')) || editors[editors.length - 1] || null; if (!editor) return { ok: false, status: 'CHAT_TITLE_UI_RENAME_EDITOR_NOT_FOUND', chat_id: chatId, desired_title: desiredTitle, editor_count: editors.length, href: location.href, title: document.title }; const applied = setValue(editor, desiredTitle); await delay(300); const confirmButtons = Array.from(document.querySelectorAll('button, [role=\"button\"]')).filter(visible); const confirmButton = confirmButtons.find((node) => /save|done|rename|confirm|ok/i.test(clean(node.innerText || node.textContent || node.getAttribute('aria-label') || node.getAttribute('title') || '')) && !/cancel/i.test(clean(node.innerText || node.textContent || node.getAttribute('aria-label') || node.getAttribute('title') || ''))); if (confirmButton) confirmButton.click(); else editor.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', code: 'Enter', bubbles: true, cancelable: true })); await delay(1000); return { ok: applied, status: applied ? 'CHAT_TITLE_UI_RENAME_ATTEMPTED' : 'CHAT_TITLE_UI_RENAME_INPUT_NOT_APPLIED', chat_id: chatId, desired_title: desiredTitle, confirm_button_used: Boolean(confirmButton), href: location.href, title: document.title }; })()`;
}
