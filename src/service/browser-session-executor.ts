...
export async function enforceChatGptReasoning(input: BrowserSessionOptions & { requirement: ChatGptReasoningRequirement }): Promise<Record<string, unknown>> {
  ...
  const mutationTrusted = mutation.ok === true && (mutation.status === "CHATGPT_REASONING_SELECTION_ATTEMPTED" || mutation.status === "CHATGPT_REASONING_EFFORT_SELECTION_ATTEMPTED");
  const explicitMismatch = after.ok === true && !verified;
  const trusted = !verified && mutationTrusted && !explicitMismatch;
  return {
    ok: verified || trusted,
    status: verified ? "CHATGPT_REASONING_REQUIREMENT_ENFORCED" : (trusted ? "CHATGPT_REASONING_TRUSTED_MUTATION" : (explicitMismatch ? "CHATGPT_REASONING_REQUIREMENT_MISMATCH" : "CHATGPT_REASONING_REQUIREMENT_UNVERIFIED")),
    verification: verified ? "VERIFIED" : (trusted ? "TRUSTED_MUTATION" : (explicitMismatch ? "MISMATCH" : "UNVERIFIED")),
...
export function reasoningRequirementSatisfied(observation: Record<string, unknown>, requirement: ChatGptReasoningRequirement): boolean {
  ...
  const modeSatisfied = mode === requirement.mode || (mode === "unknown" && effortSatisfied);
  if (!modeSatisfied) return false;
  if (!reasoningModelRequirementSatisfied(observation, requirement)) return false;
  return effortSatisfied;
}
...
function reasoningModelRequirementSatisfied(observation: Record<string, unknown>, requirement: ChatGptReasoningRequirement): boolean {
  ...
  if (requirement.model === "gpt-5.5" && /(?:gpt[-\s]*)?5\.5\b/.test(normalized)) return true;
  const effort = asString(observation.observed_effort) as ChatGptReasoningEffort | null;
  const effortOnlyLabel = /\bmedium\b|\bhigh\b|extra high|\blow\b/.test(normalized) && !/gpt|model|instant|agent|mini|legacy|temporary/.test(normalized);
  return requirement.model === "gpt-5.5" && effort !== null && effort !== "unknown" && effortOnlyLabel;
}
...
export async function dismissChatGptStorageQuotaDialog(input: BrowserSessionOptions & { targetId: string }): Promise<Record<string, unknown>> {
  const selected = await resolveTargetForInspection(input);
  if (!selected.ok || !selected.target) return { ...selected, ok: false, dismissed: false, status: "STORAGE_QUOTA_DIALOG_TARGET_NOT_READY" };
  const target = selected.target;
  if (!target.web_socket_debugger_url) return { ok: false, dismissed: false, status: "STORAGE_QUOTA_DIALOG_WEBSOCKET_MISSING", selected: compactChatGptTarget(target) };
  const result = asRecord(await safeEvaluateInTarget(
    target.web_socket_debugger_url,
    buildStorageQuotaDialogDismissExpression(),
    Math.min(Math.max(normalizeTimeout(input.timeoutMs), 2000), 10000),
    "STORAGE_QUOTA_DIALOG_DISMISS_EVALUATION_FAILED",
  ));
  return { ...result, ok: result.dismissed === true || result.status === "STORAGE_QUOTA_DIALOG_NOT_PRESENT", selected: compactChatGptTarget(target), target_id: target.id ?? null, port: target.port };
}
...
function buildResetPersistedComposerDraftExpression(): string {
  return `(() => {
    ...
    const active = document.activeElement; const activeEditable = active && editable(active) && visible(active) ? active : null; const target = activeEditable || candidates[0] || null;
...
function buildReasoningInspectionExpression(): string {
  return `(() => { const clean = (value) => String(value || '').replace(/\\s+/g, ' ').trim(); const visible = (node) => { if (!(node instanceof Element)) return false; const rect = node.getBoundingClientRect(); const style = getComputedStyle(node); return rect.width > 0 && rect.height > 0 && style.visibility !== 'hidden' && style.display !== 'none' && style.opacity !== '0'; }; const textOf = (node) => clean([node.getAttribute?.('aria-label'), node.getAttribute?.('title'), node.getAttribute?.('data-testid'), node.innerText, node.textContent].filter(Boolean).join(' ')); const badLabel = (text) => /\\bpin\\b|sidebar|history|conversation|delete chat|archive|rename|share|library|account|profile|user menu|settings|oleksandr/i.test(text); const composer = document.querySelector('textarea[data-testid="prompt-textarea"], #prompt-textarea, .ProseMirror[contenteditable="true"], div[contenteditable="true"][role="textbox"], [data-testid="prompt-textarea"]'); const surface = composer ? (composer.closest('[data-testid*=composer], [class*=composer]') || composer.closest('form') || composer.closest('main') || document) : document; const composerRect = composer?.getBoundingClientRect?.() || null; const nearComposer = (node) => { const rect = node.getBoundingClientRect(); if (node.closest('nav, aside, [data-testid*=sidebar], [class*=sidebar]')) return false; if (surface !== document && surface.contains(node)) return true; if (composerRect && Math.abs(rect.top - composerRect.top) < 180 && rect.left > window.innerWidth * 0.12) return true; return rect.top > window.innerHeight * 0.35 && rect.left > window.innerWidth * 0.12; }; const rawControls = Array.from(surface.querySelectorAll('button, [role="button"], [role="combobox"]')).concat(Array.from(document.querySelectorAll('button, [role="button"], [role="combobox"]')).filter(nearComposer)); const controls = Array.from(new Set(rawControls)).filter(visible).map((node) => ({ text: textOf(node), rect: (() => { const r = node.getBoundingClientRect(); return { left: Math.round(r.left), top: Math.round(r.top), width: Math.round(r.width), height: Math.round(r.height) }; })() })).filter((item) => item.text.length > 0 && !badLabel(item.text)); const modelControl = controls.find((item) => /gpt[-\\s]*[0-9]|5\\.5|model|thinking|reasoning|instant|\\bmedium\\b|\\bhigh\\b|extra high|\\blow\\b/i.test(item.text)) || null; const source = String(modelControl?.text || controls.map((item) => item.text).join(' | ')); const mode = /agent mode|\\bagent\\b/i.test(source) ? 'agent' : (/thinking|reasoning/i.test(source) ? 'thinking' : (/instant/i.test(source) ? 'instant' : 'unknown')); const effort = /extra high|xhigh|\\bhigh\\b/i.test(source) ? 'high' : (/\\bmedium\\b/i.test(source) ? 'medium' : (/\\blow\\b/i.test(source) ? 'low' : 'unknown')); return { ok: mode !== 'unknown' && effort !== 'unknown', status: mode !== 'unknown' && effort !== 'unknown' ? 'CHATGPT_REASONING_INSPECTED' : 'CHATGPT_REASONING_UNRESOLVED', observed_mode: mode, observed_effort: effort, observed_model_label: modelControl?.text || source || null, control_sample: controls.slice(0, 30), href: location.href, title: document.title, readyState: document.readyState }; })()`;
}
...
function buildReasoningSelectionExpression(effort: "medium" | "high", model?: "gpt-5.5"): string {
  ...
  return `(async () => { const expectedEffort = ${expectedEffort}; const expectedModel = ${expectedModel}; const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms)); const clean = (value) => String(value || '').replace(/\\s+/g, ' ').trim(); const visible = (node) => { if (!(node instanceof Element)) return false; const rect = node.getBoundingClientRect(); const style = getComputedStyle(node); return rect.width > 0 && rect.height > 0 && style.visibility !== 'hidden' && style.display !== 'none' && style.opacity !== '0'; }; const label = (node) => clean([node.getAttribute?.('aria-label'), node.getAttribute?.('title'), node.getAttribute?.('data-testid'), node.innerText, node.textContent].filter(Boolean).join(' ')).toLowerCase(); const badLabel = (text) => /\\bpin\\b|sidebar|history|conversation|delete chat|archive|rename|share|library|project|projects|recents|pinned|chats|show more|open project|open conversation|send|submit|stop|account|profile|user menu|settings|oleksandr/.test(text); const composer = document.querySelector('textarea[data-testid="prompt-textarea"], #prompt-textarea, .ProseMirror[contenteditable="true"], div[contenteditable="true"][role="textbox"], [data-testid="prompt-textarea"]'); const surface = composer ? (composer.closest('[data-testid*=composer], [class*=composer]') || composer.closest('form') || composer.closest('main') || document) : document; const composerRect = composer?.getBoundingClientRect?.() || null; const nearComposer = (node) => { const rect = node.getBoundingClientRect(); if (node.closest('nav, aside, [data-testid*=sidebar], [class*=sidebar]')) return false; if (surface !== document && surface.contains(node)) return true; if (composerRect && Math.abs(rect.top - composerRect.top) < 180 && rect.left > window.innerWidth * 0.12) return true; return rect.top > window.innerHeight * 0.35 && rect.left > window.innerWidth * 0.12; }; const scopedControls = () => Array.from(new Set(Array.from(surface.querySelectorAll('button, [role="button"], [role="combobox"]')).concat(Array.from(document.querySelectorAll('button, [role="button"], [role="combobox"]')).filter(nearComposer)))).filter(visible); const menuControls = () => Array.from(document.querySelectorAll('button, [role="button"], [role="combobox"], [role="menuitem"], [role="option"]')).filter(visible); const effortLabel = (text) => text.includes('medium') || text.includes('extra high') || /(^|\\s)high(\\s|$)/.test(text) || /(^|\\s)low(\\s|$)/.test(text); const structuralPicker = (node) => { const rect = node.getBoundingClientRect(); const text = label(node); const inComposerBand = composerRect && Math.abs(rect.top - composerRect.top) < 180 && rect.left > window.innerWidth * 0.25 && rect.top > window.innerHeight * 0.45; const bottomBand = rect.top > window.innerHeight * 0.60 && rect.left > window.innerWidth * 0.25; return (inComposerBand || bottomBand) && (node.getAttribute('aria-haspopup') === 'menu' || node.getAttribute('aria-expanded') !== null || node.getAttribute('role') === 'combobox') && !badLabel(text); }; const scoped = scopedControls(); const picker = scoped.find((node) => /model|gpt[-\\s]*[0-9]|5\\.5|instant|thinking|reasoning/.test(label(node)) && !badLabel(label(node))) || scoped.find((node) => effortLabel(label(node)) && !badLabel(label(node))) || scoped.find(structuralPicker) || null; if (!picker) return { ok: false, status: 'CHATGPT_REASONING_PICKER_NOT_FOUND', requested_model: expectedModel, requested_effort: expectedEffort, control_sample: scoped.map((node) => label(node)).filter(Boolean).slice(0, 30) }; const pickerLabel = label(picker); const pickerIsEffort = effortLabel(pickerLabel) && !/model|gpt[-\\s]*[0-9]|5\\.5|instant|thinking|reasoning/.test(pickerLabel); picker.click(); await delay(500); let modelControl = null; if (pickerIsEffort) { const effortPattern = expectedEffort === 'high' ? /extra high|xhigh|\\bhigh\\b/ : /\\bmedium\\b/; const effortControl = menuControls().find((node) => effortPattern.test(label(node)) && !/instant|agent|pin|sidebar|history|conversation/.test(label(node))) || (effortPattern.test(pickerLabel) ? picker : null); if (!effortControl) return { ok: false, status: 'CHATGPT_REASONING_EFFORT_OPTION_NOT_FOUND', requested_model: expectedModel, requested_effort: expectedEffort, picker_label: pickerLabel, model_label: null, thinking_label: null, control_sample: menuControls().map((node) => label(node)).filter(Boolean).slice(0, 50) }; if (effortControl !== picker) { effortControl.click(); await delay(600); } return { ok: true, status: 'CHATGPT_REASONING_EFFORT_SELECTION_ATTEMPTED', requested_mode: 'thinking', requested_model: expectedModel, requested_effort: expectedEffort, picker_label: pickerLabel, model_label: null, thinking_label: null, effort_label: label(effortControl) }; } if (expectedModel === 'gpt-5.5') { const modelPattern = /(?:gpt[-\\s]*)?5\\.5\\b|\\b5\\.5\\b/; modelControl = menuControls().find((node) => modelPattern.test(label(node)) && !/mini|legacy|temporary|pin|sidebar|history|conversation/.test(label(node))) || null; if (modelControl) { modelControl.click(); await delay(600); } } const thinking = menuControls().find((node) => /thinking|reasoning/.test(label(node)) && !/instant|agent|pin|sidebar|history|conversation/.test(label(node))) || null; if (!thinking) return { ok: false, status: 'CHATGPT_REASONING_THINKING_OPTION_NOT_FOUND', requested_model: expectedModel, requested_effort: expectedEffort, picker_label: label(picker), model_label: modelControl ? label(modelControl) : null, control_sample: menuControls().map((node) => label(node)).filter(Boolean).slice(0, 50) }; thinking.click(); await delay(500); const effortPattern = expectedEffort === 'high' ? /extra high|xhigh|\\bhigh\\b/ : /\\bmedium\\b/; const effortControl = menuControls().find((node) => effortPattern.test(label(node)) && !/instant|agent|pin|sidebar|history|conversation/.test(label(node))) || null; if (!effortControl) return { ok: false, status: 'CHATGPT_REASONING_EFFORT_OPTION_NOT_FOUND', requested_model: expectedModel, requested_effort: expectedEffort, picker_label: label(picker), model_label: modelControl ? label(modelControl) : null, thinking_label: label(thinking), control_sample: menuControls().map((node) => label(node)).filter(Boolean).slice(0, 50) }; effortControl.click(); await delay(600); return { ok: true, status: 'CHATGPT_REASONING_SELECTION_ATTEMPTED', requested_mode: 'thinking', requested_model: expectedModel, requested_effort: expectedEffort, picker_label: label(picker), model_label: modelControl ? label(modelControl) : null, thinking_label: label(thinking), effort_label: label(effortControl) }; })()`;
}

function buildStorageQuotaDialogDismissExpression(): string {
  return `(async () => {
    const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
    const clean = (value) => String(value || '').replace(/\\s+/g, ' ').trim();
    const visible = (node) => { if (!(node instanceof Element)) return false; const rect = node.getBoundingClientRect(); const style = getComputedStyle(node); return rect.width > 0 && rect.height > 0 && style.display !== 'none' && style.visibility !== 'hidden' && style.opacity !== '0'; };
    const textOf = (node) => clean([node.innerText, node.textContent, node.getAttribute?.('aria-label'), node.getAttribute?.('title')].filter(Boolean).join(' '));
    const quotaPattern = /(not enough|insufficient|ran out of|out of).{0,50}(storage|disk|memory|space)|(?:storage|disk|memory).{0,50}(full|limit|quota|space)|free up.{0,40}(storage|disk|memory|space)|optimi[sz]e.{0,40}(storage|disk|memory|space)|buy.{0,30}(more|additional).{0,30}(storage|disk|memory|space)|upgrade.{0,40}(storage|disk|memory|space)|(?:не хватает|недостаточно|закончилось|переполнено).{0,50}(места|памяти|хранилища|диска)|(?:место|память|хранилище|диск).{0,50}(закончилось|переполнено|лимит)/i;
    const surfaces = Array.from(document.querySelectorAll('[role="dialog"], [aria-modal="true"]')).filter(visible);
    const dialog = surfaces.find((node) => quotaPattern.test(textOf(node))) || null;
    if (!dialog) return { ok: true, dismissed: false, status: 'STORAGE_QUOTA_DIALOG_NOT_PRESENT', dialog_count: surfaces.length, href: location.href, title: document.title };
    const controls = Array.from(dialog.querySelectorAll('button, [role="button"]')).filter(visible);
    const label = (node) => textOf(node).toLowerCase();
    const close = controls.find((node) => /^(close|dismiss|not now|cancel|x)$/.test(label(node)))
      || controls.find((node) => /close|dismiss|not now|cancel/.test(label(node)))
      || controls.find((node) => /close/.test(String(node.getAttribute('aria-label') || node.getAttribute('title') || '').toLowerCase()))
      || null;
    if (close) close.click();
    else document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
    await delay(350);
    const stillVisible = visible(dialog) && document.contains(dialog);
    return { ok: !stillVisible, dismissed: !stillVisible, status: !stillVisible ? 'STORAGE_QUOTA_DIALOG_DISMISSED' : 'STORAGE_QUOTA_DIALOG_DISMISS_NOT_CONFIRMED', method: close ? 'close_control' : 'escape', dialog_text_sample: textOf(dialog).slice(0, 300), control_labels: controls.map(label).filter(Boolean).slice(0, 20), href: location.href, title: document.title };
  })()`;
}

function buildComposerPreflightExpression(): string {
  return `(() => { const composerSelectors = ['textarea[data-testid="prompt-textarea"]', '#prompt-textarea', 'textarea', '.ProseMirror[contenteditable="true"]', 'div[contenteditable="true"][role="textbox"]', 'main form textarea', 'main form [contenteditable="true"]', '[data-testid="prompt-textarea"]']; const sendSelectors = ['button[data-testid="send-button"]', 'button[data-testid="composer-submit-button"]', 'button[data-testid*="send" i]', 'button[data-testid*="submit" i]', 'button[aria-label="Send prompt"]', 'button[aria-label="Send message"]', 'button[aria-label*="send" i]', 'button[aria-label*="submit" i]', '#composer-submit-button', 'form button[type="submit"]']; const readText = (node) => { if (!node) return ''; if ('value' in node) return String(node.value || ''); const rendered = String(node.innerText || ''); if (rendered.length > 0) return rendered; const clone = node.cloneNode(true); for (const excluded of clone.querySelectorAll?.('[contenteditable="false"], button, input, [data-testid*=attachment], [data-testid*=file], [class*=attachment], [class*=file], [aria-label*=attachment i], [aria-label*=file i]') || []) excluded.remove(); return String(clone.textContent || ''); }; const visible = (node) => { if (!node || !(node instanceof Element)) return false; const rect = node.getBoundingClientRect(); const style = getComputedStyle(node); return rect.width > 0 && rect.height > 0 && style.visibility !== 'hidden' && style.display !== 'none' && style.opacity !== '0'; }; const composerCandidates = composerSelectors.map((selector) => document.querySelector(selector)).filter(Boolean); let composerNode = composerCandidates.find((node) => node instanceof HTMLTextAreaElement || node.getAttribute('contenteditable') === 'true' || node.classList.contains('ProseMirror')); if (composerNode && !(composerNode instanceof HTMLTextAreaElement) && composerNode.getAttribute('contenteditable') !== 'true' && composerNode.querySelector) composerNode = composerNode.querySelector('textarea, [contenteditable="true"], .ProseMirror'); const composerContainer = composerNode ? (composerNode.closest('form') || composerNode.closest('[data-testid*=composer], [class*=composer], main') || document) : document; const explicitSendNode = sendSelectors.map((selector) => document.querySelector(selector)).filter(Boolean).find(visible) || null; const nearbyButtons = Array.from((composerContainer || document).querySelectorAll('button')).filter((node) => visible(node)); const enabledNearbyButtons = nearbyButtons.filter((node) => !node.disabled && node.getAttribute('aria-disabled') !== 'true'); const sendNode = explicitSendNode || enabledNearbyButtons.find((node) => { const label = String(node.getAttribute('aria-label') || node.getAttribute('title') || node.getAttribute('data-testid') || node.innerText || node.textContent || '').toLowerCase(); if (label.includes('send') || label.includes('submit') || label.includes('arrow')) return true; const svgCount = node.querySelectorAll('svg').length; const text = String(node.innerText || node.textContent || '').trim(); return svgCount > 0 && text.length <= 40; }) || null; const composerRect = composerNode && composerNode.getBoundingClientRect ? composerNode.getBoundingClientRect() : null; const sendRect = sendNode && sendNode.getBoundingClientRect ? sendNode.getBoundingClientRect() : null; const intersects = (a, b) => a && b && a.left < b.right && a.right > b.left && a.top < b.bottom && a.bottom > b.top; const viewportArea = Math.max(1, window.innerWidth * window.innerHeight); const blockers = Array.from(document.querySelectorAll('[role="dialog"], [aria-modal="true"], [data-radix-popper-content-wrapper], [data-headlessui-state], .fixed, .absolute')).filter((node) => visible(node)).map((node) => { const rect = node.getBoundingClientRect(); const style = getComputedStyle(node); const z = Number.parseInt(style.zIndex || '0', 10) || 0; const area = rect.width * rect.height; const coversComposer = Boolean(intersects(rect, composerRect) || intersects(rect, sendRect)); const modal = node.getAttribute('aria-modal') === 'true' || node.getAttribute('role') === 'dialog'; const pointerEvents = String(style.pointerEvents || '').toLowerCase(); const passThrough = pointerEvents === 'none'; const highLayer = (style.position === 'fixed' || style.position === 'absolute') && z >= 20 && area > 5000 && !passThrough; return { node, rect, style, z, area, coversComposer, modal, highLayer, pointerEvents, passThrough }; }).filter((item) => (item.modal || item.highLayer) && (item.coversComposer || item.area > viewportArea * 0.15)).sort((a, b) => (b.modal === a.modal ? b.z - a.z : (b.modal ? 1 : -1))); const blocker = blockers[0] || null; const sendDisabled = sendNode ? Boolean(sendNode.disabled) || sendNode.getAttribute('aria-disabled') === 'true' : true; const composerText = composerNode ? readText(composerNode).trim() : ''; const overlayText = blocker ? String(blocker.node.innerText || blocker.node.textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 300) : ''; const messageNodes = Array.from(document.querySelectorAll('[data-message-author-role]')).filter(visible); const userMessages = messageNodes.filter((node) => node.getAttribute('data-message-author-role') === 'user'); const assistantMessages = messageNodes.filter((node) => node.getAttribute('data-message-author-role') === 'assistant'); const visibleTextSample = String(document.body?.innerText || document.documentElement?.innerText || '').replace(/\\s+/g, ' ').trim().slice(0, 300); const temporaryChat = visibleTextSample.toLowerCase().includes('temporary chat'); const overlay = blocker ? { present: true, role: blocker.node.getAttribute('role'), ariaModal: blocker.node.getAttribute('aria-modal'), zIndex: blocker.z, coversComposer: blocker.coversComposer, pointerEvents: blocker.pointerEvents, passThrough: blocker.passThrough, textSample: overlayText, tag: blocker.node.tagName, className: String(blocker.node.className || '').slice(0, 200) } : { present: false }; const composer = { found: Boolean(composerNode), visible: Boolean(composerNode && visible(composerNode)), textLength: composerText.length, candidateCount: composerCandidates.length, active: document.activeElement === composerNode }; const sendControl = { found: Boolean(sendNode), enabled: Boolean(sendNode && !sendDisabled), disabled: sendDisabled }; const ok = composer.found && composer.visible && sendControl.found && sendControl.enabled && overlay.present !== true; return { ok, status: ok ? 'COMPOSER_PREFLIGHT_READY' : (overlay.present ? 'COMPOSER_PREFLIGHT_BLOCKED_OVERLAY' : 'COMPOSER_PREFLIGHT_NOT_READY'), composer, sendControl, overlay, href: location.href, title: document.title, visible_text_sample: visibleTextSample, message_count: messageNodes.length, user_message_count: userMessages.length, assistant_message_count: assistantMessages.length, temporary_chat: temporaryChat, readyState: document.readyState, visibility_state: document.visibilityState, has_focus: document.hasFocus(), hidden: document.hidden }; })()`;
}
