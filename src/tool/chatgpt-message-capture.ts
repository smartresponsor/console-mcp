...
function normalizeConversationState(raw: unknown, maxMessages: number): NormalizedConversationState {
  ...
  const activitySignature = String(source.activitySignature ?? `${latestAssistant?.hash ?? "no-assistant"}:${composerActionMode}:${composerControlReason}:${sidebarActivityMode}:${String(source.toolText ?? "")}:${String(source.submitDisabled ?? "")}:${messages.length}`);
  return { messages, latestAssistant, busy, activitySignature, composerActionMode, composerControlReason, composerControlCount, visibleComposerControlCount, hiddenComposerControlCount, sidebarActivityMode, sidebarActivityReason };
}
