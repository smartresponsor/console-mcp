...
async function settleChatGptAnswer(input: z.infer<typeof answerSettleInputSchema>): Promise<Record<string, unknown>> {
  ...
  return { ok: false, status: finalStatus, settled: false, ready_for_gate: false, selected: target, scans: tabResult.scans, messages: lastState?.messages ?? [], latest_assistant: lastState?.latestAssistant ?? null, stability: { stable_samples: stableSamples, min_stable_samples: timing.minStableSamples, busy: lastState?.busy ?? null, composer_action_mode: finalComposerMode, require_composer_send_mode: input.requireComposerSendMode, idle_quiet_ms: timing.idleQuietMs, waited_ms: Date.now() - observationStartedAt, observation_budget_ms: timing.observationBudgetMs, max_wait_ms: timing.maxWaitMs }, policy: buildAnswerSettlePolicy() };
}
...
function buildConversationStateExpression(maxMessages: number): string { return `(() => { const nodes = Array.from(document.querySelectorAll('[data-message-author-role]')); const messages = nodes.map((node, index) => { const role = String(node.getAttribute('data-message-author-role') || 'unknown'); const text = String((node.innerText || node.textContent || '')).trim(); return { role, text, index }; }).filter((item) => item.text.length > 0).slice(-${maxMessages}); const controlSelectors = ['button[data-testid="stop-button"]', 'button[data-testid="composer-stop-button"]', 'button[data-testid="send-button"]', 'button[data-testid="composer-submit-button"]', 'button[aria-label="Stop generating"]', 'button[aria-label="Stop streaming"]', 'button[aria-label="Send prompt"]', 'button[aria-label="Send message"]', 'form button[type="submit"]']; const controls = controlSelectors.flatMap((selector) => Array.from(document.querySelectorAll(selector))).filter(Boolean); const controlText = controls.map((node) => String(node.innerText || node.textContent || node.getAttribute('aria-label') || node.getAttribute('data-testid') || '')).join('|').toLowerCase(); const controlHtml = controls.map((node) => String(node.outerHTML || '')).join('|').toLowerCase().slice(0, 4000); const stopMode = controlText.includes('stop') || controlText.includes('останов') || controlHtml.includes('stop-button') || controlHtml.includes('composer-stop-button') || controlHtml.includes('stop generating') || controlHtml.includes('stop streaming'); const sendMode = controlText.includes('send') || controlText.includes('отправ') || controlHtml.includes('send-button') || controlHtml.includes('composer-submit-button') || controlHtml.includes('send prompt') || controlHtml.includes('send message'); const primaryControl = controls[0] || null; const submitDisabled = primaryControl ? Boolean(primaryControl.disabled) || primaryControl.getAttribute('aria-disabled') === 'true' : null; const composerActionMode = stopMode ? 'stop' : (sendMode ? (submitDisabled ? 'disabled' : 'send') : 'unknown'); const busySelectors = ['[data-testid*="tool"]', '[aria-label*="tool" i]', '[class*="tool" i]', '[class*="progress" i]', '[class*="spinner" i]']; const busyNodes = busySelectors.flatMap((selector) => Array.from(document.querySelectorAll(selector))).filter(Boolean); const toolText = busyNodes.map((node) => String(node.innerText || node.textContent || node.getAttribute('aria-label') || '')).join('|').slice(0, 2000); const toolBusy = busyNodes.some((node) => { const text = String(node.innerText || node.textContent || node.getAttribute('aria-label') || '').toLowerCase(); return text.includes('running') || text.includes('working') || text.includes('calling') || text.includes('searching') || text.includes('reading') || text.includes('using') || text.includes('in progress') || text.includes('подожд') || text.includes('выполня'); }); const generating = stopMode || toolBusy; const latestAssistant = [...messages].reverse().find((item) => item.role === 'assistant'); const activitySignature = [messages.length, latestAssistant ? latestAssistant.text.length : 0, latestAssistant ? latestAssistant.text.slice(-200) : '', composerActionMode, generating, toolText, submitDisabled].join('::'); return { messages, generating, toolBusy, toolText, composerActionMode, activitySignature, submitDisabled, readyState: document.readyState, href: location.href, title: document.title }; })()`; }
...
function normalizeConversationState(raw: unknown, maxMessages: number): NormalizedConversationState {
  ...
  const activitySignature = String(source.activitySignature ?? `${latestAssistant?.hash ?? "no-assistant"}:${composerActionMode}:${String(source.toolText ?? "")}:${String(source.submitDisabled ?? "")}:${messages.length}`);
  return { messages, latestAssistant, busy, activitySignature, composerActionMode };
}
