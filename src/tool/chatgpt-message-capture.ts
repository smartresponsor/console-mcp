...
function buildMessageDomExpression(maxMessages: number): string { return `(() => { const nodes = Array.from(document.querySelectorAll('[data-message-author-role]')); const items = nodes.map((node, index) => { const role = String(node.getAttribute('data-message-author-role') || 'unknown'); const explicitContent = role === 'assistant' ? node.querySelector('[data-message-content]') : null; const markdownRoots = role === 'assistant' && !explicitContent ? Array.from(node.querySelectorAll('.markdown')).filter((candidate) => !candidate.parentElement?.closest('.markdown')) : []; const assistantText = explicitContent ? String(explicitContent.innerText || explicitContent.textContent || '') : (markdownRoots.length > 0 ? markdownRoots.map((candidate) => String(candidate.innerText || candidate.textContent || '').trim()).filter(Boolean).join(String.fromCharCode(10)) : ''); const text = String(((role === 'assistant' && assistantText.length > 0 ? assistantText : (node.innerText || node.textContent)) || '')).trim(); return { role, text, index }; }).filter((item) => item.text.length > 0); return items.slice(Math.max(0, items.length - ${maxMessages})); })()`; }
