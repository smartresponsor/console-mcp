...
async function settleChatGptAnswer(input: z.infer<typeof answerSettleInputSchema>): Promise<Record<string, unknown>> {
  ...
  while (Date.now() <= deadline) {
    ...
    if (hasNewAssistant && stableSamples >= timing.minStableSamples) {
      const binding = target.chat_id === null ? null : createChatGptSessionBinding({ url: target.url ?? "", boundAt: new Date().toISOString(), baselineAssistantHash: latestAssistant?.hash ?? null });
      return { ok: true, status: "ANSWER_STABLE", settled: true, ready_for_gate: true, selected: target, scans: tabResult.scans, binding, cursor: binding === null ? null : createChatGptArtifactCursor(binding), messages: state.messages, latest_assistant: latestAssistant, stability: { stable_samples: stableSamples, min_stable_samples: timing.minStableSamples, busy: state.busy, composer_action_mode: state.composerActionMode, composer_control_reason: state.composerControlReason, composer_control_count: state.composerControlCount, visible_composer_control_count: state.visibleComposerControlCount, hidden_composer_control_count: state.hiddenComposerControlCount, sidebar_activity_mode: state.sidebarActivityMode, sidebar_activity_reason: state.sidebarActivityReason, animated_status_mode: state.animatedStatusMode, animated_status_reason: state.animatedStatusReason, animated_status_text: state.animatedStatusText, tail_activity_mode: state.animatedStatusMode, tail_activity_reason: state.animatedStatusReason, tail_activity_text: state.animatedStatusText, require_composer_send_mode: input.requireComposerSendMode, idle_quiet_ms: timing.idleQuietMs, idle_since_ms: idleSince === null ? null : now - idleSince, waited_ms: Date.now() - observationStartedAt, observation_budget_ms: timing.observationBudgetMs, max_wait_ms: timing.maxWaitMs }, policy: buildAnswerSettlePolicy() };
    }
  ...
  return { ok: false, status: finalStatus, settled: false, ready_for_gate: false, selected: target, scans: tabResult.scans, messages: lastState?.messages ?? [], latest_assistant: lastState?.latestAssistant ?? null, stability: { stable_samples: stableSamples, min_stable_samples: timing.minStableSamples, busy: lastState?.busy ?? null, composer_action_mode: finalComposerMode, composer_control_reason: lastState?.composerControlReason ?? null, composer_control_count: lastState?.composerControlCount ?? null, visible_composer_control_count: lastState?.visibleComposerControlCount ?? null, hidden_composer_control_count: lastState?.hiddenComposerControlCount ?? null, sidebar_activity_mode: lastState?.sidebarActivityMode ?? null, sidebar_activity_reason: lastState?.sidebarActivityReason ?? null, animated_status_mode: lastState?.animatedStatusMode ?? null, animated_status_reason: lastState?.animatedStatusReason ?? null, animated_status_text: lastState?.animatedStatusText ?? null, tail_activity_mode: lastState?.animatedStatusMode ?? null, tail_activity_reason: lastState?.animatedStatusReason ?? null, tail_activity_text: lastState?.animatedStatusText ?? null, require_composer_send_mode: input.requireComposerSendMode, idle_quiet_ms: timing.idleQuietMs, waited_ms: Date.now() - observationStartedAt, observation_budget_ms: timing.observationBudgetMs, max_wait_ms: timing.maxWaitMs }, policy: buildAnswerSettlePolicy() };
}
...
function normalizeConversationState(raw: unknown, maxMessages: number): NormalizedConversationState {
  ...
  return { messages, latestAssistant, busy, activitySignature, composerActionMode, composerControlReason, composerControlCount, visibleComposerControlCount, hiddenComposerControlCount, sidebarActivityMode, sidebarActivityReason, animatedStatusMode, animatedStatusReason, animatedStatusText };
}
