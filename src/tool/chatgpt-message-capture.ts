...
type ClientStreamErrorSignal = { detected: boolean; mode: string; text: string | null; retry_visible: boolean; connecting_to_app: boolean; error_in_message_stream: boolean; delivery_timed_out: boolean };
...
function normalizeClientStreamErrorSignal(raw: unknown): ClientStreamErrorSignal {
  ...
  const detected = source.detected === true || errorInMessageStream || deliveryTimedOut || (connectingToApp && retryVisible);
  return {
    detected,
